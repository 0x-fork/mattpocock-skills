...
3. **Branch: is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally (one red-green slice at a time), then closes out by running **`/code-review`**, a two-axis review (Standards + Spec) of the diff, before committing. Once that diff is committed, **`/pr`** writes the pull request body: summary from the primary source rather than the diff, the shape of the change, evidence tied to the failure it guards against, what was left out, and a one-way/two-way door call, then opens it. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, **`/code-review`** on its own whenever you want to review a branch or PR against a fixed point, and **`/pr`** on its own whenever you're opening a pull request and want the body written properly rather than the generic default.
